...
async function fetchPlaylistItemsFallbackRSS(playlistId) {
  try {
    ...
    const items = entries.map((entry) => {
      ...
      return { contentDetails: { videoId }, snippet: { title } };
    });
    return items;
  } catch (e) {
...
async function fetchAllPlaylistItemsWithApi(playlistId, apiKey) {
  let allItems = [];
  let nextPageToken = '';
  do {
    ...
    const getCourse = async () => {
      try{
        ...
        const allItems = useApi
          ? await fetchAllPlaylistItemsWithApi(courseId, apiKey)
