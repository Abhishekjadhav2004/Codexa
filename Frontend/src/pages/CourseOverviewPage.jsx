...
// Fallback: fetch playlist items via public RSS when API key is missing
async function fetchPlaylistItemsFallbackRSS(playlistId) {
  try {
...
async function fetchPlaylistItemsWithApi(playlistId, apiKey) {
...
const CourseOverviewPage = () => {
  ...
  useEffect(() => {
    const getCourse = async () => {
      try{
        const apiKey = import.meta.env.VITE_YT_V3_API_KEY;
        const useApi = typeof apiKey === 'string' && apiKey.trim().length > 0;
        const items = useApi
          ? await fetchPlaylistItemsWithApi(courseId, apiKey)
          : await fetchPlaylistItemsFallbackRSS(courseId);
