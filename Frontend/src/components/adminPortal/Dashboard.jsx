...
const Dashboard = () => {
  ...
  const noProblems = data.noProblems || 0;
  const noSubmissions = data.noSubmissions || 0;
  const noUsers = data.noUsers || 0;
  const latestSubmissions = data.latestSubmissions || [];

  if (!data) {
    return <p className="text-center text-gray-500">Loading Admin Panel...</p>;
  }
            ...
            {latestSubmissions.length > 0 ? (
              latestSubmissions.map((submission) => (
                <RecentSubmissionItem
                  key={submission._id}
                  title={submission.problemId?.title || "Untitled"}
                  user={submission.userId?.username || "Unknown"}
                  status={getProcessedStatus(submission.status)}
                  time={getTimeStamp(submission.createdAt)}
                />
              ))
            ) : (
