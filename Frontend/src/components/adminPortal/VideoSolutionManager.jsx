...
const VideoSolutionManager = () => {
  ...
  useEffect(() => {
    if (!data || !data.videoSolutions || !data.problems) {
      console.error("VideoSolutionManager: Missing data from outlet context", data);
      return;
    }
    ...
    const filteredProblems = data.problems.filter(p => p?._id && !problemHasSoln[p._id]);
    setProblems(filteredProblems);
    setProblemsWithVideoSolutions(data.videoSolutions.filter(soln => soln?.problem?._id));
  }, [data]);
  ...
  if (!data || !data.videoSolutions || !data.problems) {
    return <p className="text-center text-gray-500 mt-4">Loading video solutions or access denied.</p>;
  }
  ...
  return (
    <>
      <div className="bg-[#FFFFFF] dark:bg-[#1E293B] p-6 rounded-xl border border-[#E2E8F0] dark:border-[#334155] shadow-md">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#0F172A] dark:text-[#F8FAFC]">Video Solutions</h2>
          <div className="relative flex w-full max-w-sm rounded-lg p-1 bg-gray-100 border border-gray-300 dark:bg-gray-800 dark:border-gray-700">
            <div className={`absolute top-1 bottom-1 w-[48%] rounded-lg transition-all duration-300 bg-orange-500 ${selectedOption === "add" ? "left-1" : "left-[51%]"}`}></div>
            <button
              onClick={() => setSelectedOption("add")}
              className={`z-10 flex-1 py-2 text-sm font-semibold rounded-lg transition-colors duration-300 ${selectedOption === "add" ? "text-white" : "text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"}`}
            >
              Add Solution
            </button>
            <button
              onClick={() => setSelectedOption("delete")}
              className={`z-10 flex-1 py-2 text-sm font-semibold rounded-lg transition-colors duration-300 ${selectedOption === "delete" ? "text-white" : "text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"}`}
            >
              Delete Solution
            </button>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            {selectedOption === "add" && (
              <tbody>
                {(problems || []).map((problem) => (
                  <tr key={problem._id || Math.random()} className="border-b border-[#E2E8F0] dark:border-[#334155] hover:bg-[#F1F5F9] dark:hover:bg-[#0F172A]/50 transition-colors duration-200">
                    <td className="px-6 py-4 font-medium whitespace-nowrap text-[#0F172A] dark:text-[#F8FAFC]">{problem.title || "Untitled"}</td>
                    <td className="px-6 py-4"><DifficultyBadge difficulty={problem.difficulty} /></td>
                    <td className="px-6 py-4 flex flex-wrap gap-2 max-w-xs">{(problem.tags || []).map((tag, idx) => (<span key={idx} className="bg-gray-200 dark:bg-gray-700 text-xs font-normal px-2 py-1 rounded-md">{tag}</span>))}</td>
                    <td className="px-6 py-4 text-[#0F172A] dark:text-[#F8FAFC]">{(problem.createdAt || "").split('T')[0]}</td>
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => navigate(`upload/${problem._id}`)}
            ...
            {selectedOption === "delete" && (
              <tbody>
                {(problemsWithVideoSolutions || []).map((soln) => (
                  soln.problem && (
                    <tr key={soln._id || Math.random()} className="border-b border-[#E2E8F0] dark:border-[#334155] hover:bg-[#F1F5F9] dark:hover:bg-[#0F172A]/50 transition-colors duration-200">
                      <td className="px-6 py-4 font-medium whitespace-nowrap text-[#0F172A] dark:text-[#F8FAFC]">{soln.problem.title || "Untitled"}</td>
                      <td className="px-6 py-4"><DifficultyBadge difficulty={soln.problem.difficulty} /></td>
                      <td className="px-6 py-4 flex flex-wrap gap-2 max-w-xs">{(soln.problem.tags || []).map((tag, idx) => (<span key={idx} className="bg-gray-200 dark:bg-gray-700 text-xs font-normal px-2 py-1 rounded-md">{tag}</span>))}</td>
                      <td className="px-6 py-4 text-[#0F172A] dark:text-[#F8FAFC]">{(soln.createdAt || "").split('T')[0]}</td>
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => navigate(`/problems/edit/${soln.problem._id}`)}
