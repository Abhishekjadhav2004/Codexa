...
const ProblemManager = () => {
  ...
  useEffect(() => {
    if (data && data.problems) {
      ...
      console.error("No problem data available. Possibly due to 401 Unauthorized error.");
    }
  }, [data]);
  ...
  if (!problems.length) {
    return <p className="text-center text-gray-500 mt-4">No problems found or access denied.</p>;
  }
                ...
                >
                  ...
                  <td className="px-6 py-4 flex flex-wrap gap-2 max-w-xs">
                    {(problem.tags || []).map((tag, index) => (
                      <span key={index} className="bg-gray-200 dark:bg-gray-700 text-xs font-normal px-2 py-1 rounded-md">{tag}</span>
                    ))}
                  </td>
                  <td className="px-6 py-4 text-[#0F172A] dark:text-[#F8FAFC]">
